<?php
    ...
    if($row['total'] == 0) {
        $_SESSION['msg'] = "Matrícula inválida";
        header('Location: cadastro_de_req.php');
        exit;
    }

    $sql = "INSERT INTO requerimento (req_fun_matricula, req_despacho, req_descricao, req_entrada, req_deferimento, req_volta, req_tipo_req) VALUES ('$matricula', '$despacho', '$descricao','$entrada', '$deferimento', '$volta', '$tipo_req')";

    if($conexao->query($sql) === TRUE) {
        $_SESSION['msg'] = "Requerimento cadastrado com sucesso.";
        header('Location: cadastro_de_req.php');
    exit;
    }
